...
static LRESULT CALLBACK SpinnerWindowProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	switch (uMsg)
	{
		case WM_CREATE:
		{
			...
			control->EditWindowProc = (WNDPROC)(LONG_PTR)GetWindowLong(control->hEdit, GWLP_WNDPROC);
			SetWindowLong(control->hEdit, GWLP_USERDATA, (LONG)(LONG_PTR)control);
			SetWindowLong(control->hEdit, GWLP_WNDPROC,  (LONG)(LONG_PTR)SpinnerEditWindowProc);
...
{
    DestroyCursor(SystemInfo::hNormalCursor);
    DestroyCursor(SystemInfo::hSpinCursor);
    UnregisterClass(L"Spinner", hInstance);
}